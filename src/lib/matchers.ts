...
import { ImgsrcDefaultError } from "../models/errors/imgsrcdefaulterror.js";
...
export function match<T, E>(
...
): MatchFunc<T, E | ImgsrcDefaultError | ResponseValidationError> {
  return async function matchFunc(
    ...
    [
      result: Result<T, E | ImgsrcDefaultError | ResponseValidationError>,
      raw: unknown,
    ]
  > {
    let raw: unknown;
    ...
    if (!matcher) {
      return [{
        ok: false,
        error: new ImgsrcDefaultError("Unexpected Status or Content-Type", {
          response,
          request,
    ...
    if (matcher.enc === "fail") {
      return [{
        ok: false,
        error: new ImgsrcDefaultError("API error occurred", {
          request,
          response,
          body,
        }),
